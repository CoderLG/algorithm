 ...
 *
 *
 *   思想: 二分递归  先排左边，在排右边，两个有序数组，用双指针排序
 *   双指针排序（官方叫法不明）：前提为有序，申请一块 m+n 大小的数组，利用双指针便利比较两部分，谁小谁放进新申请的数组中，
 *   有一个到达终点就停止，将未停止的那个数组后边的数 再依次放进这个数组
 *
 *   小数和问题：一个数他的左边比他小的数之和，称谓小数和，求这个数组的小数和？
 *   merge的时候，左边移动的时候，那么 右边的数都比这个数大
 *
